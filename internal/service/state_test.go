...
func TestRunningStateCaptures(t *testing.T) {
	pid := os.Getpid()
	...
	if s.PID != pid {
		t.Fatalf("PID: got %d, want %d", s.PID, pid)
	}
	if s.PGID != pid {
		t.Fatalf("PGID: got %d, want %d", s.PGID, pid)
	}
	if s.LogPath != "/tmp/test.log" {
		t.Fatalf("LogPath: got %q", s.LogPath)
	}
	if s.BootTime == 0 {
		t.Fatal("BootTime should be nonzero")
	}
	if s.CreateTime == 0 {
		t.Fatal("CreateTime should be nonzero")
	}
}
